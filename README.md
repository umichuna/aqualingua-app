...
> ⚠️ **Vercel の環境変数は、変更しただけでは動いている本番に反映されない。**
> 値はデプロイ時に焼き込まれるので、変更したら必ず **再デプロイ**する
> （Deployments → 最新 → ⋯ → Redeploy）。「環境変数は直したのに直らない」の大半はこれ。
> DBのパスワードを変える手順は `STATUS_REPORT.md` の追記10 に書いてある。
